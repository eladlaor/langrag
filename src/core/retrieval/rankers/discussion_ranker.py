...
class DiscussionRanker:
    """
    High-level interface for discussion ranking.

    This class provides a clean API for ranking discussions, encapsulating
    all the helper functions into a single interface.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            session_id: Langfuse session ID for grouping traces
            user_id: User identifier (e.g., data_source_name)
            enable_mmr: Enable MMR diversity reranking (default: True)
            mmr_lambda: MMR balance parameter (0-1, default: 0.7 = 70% quality, 30% diversity)
        """
        try:
            ...
            logger.error(f"Failed to initialize DiscussionRanker: {e}, summary_format={summary_format}")
            raise RuntimeError(f"Failed to initialize DiscussionRanker: {e}") from e

    def rank(self, discussions_file: str, output_file: str, force_refresh: bool = False) -> dict[str, Any]:
        """
        Rank discussions from file and save results.

        Args:
            discussions_file: Path to input discussions JSON
            output_file: Path to save ranking results
            force_refresh: If True, re-rank even if output exists
        ...
        try:
            ...
            if not force_refresh and os.path.exists(output_file):
                logger.info(f"Using existing discussions ranking file: {output_file}")
            ...
            if not discussions:
                logger.warning("No discussions found - creating empty ranking")
                ranking_result = {RankingResultKeys.RANKED_DISCUSSIONS: [], RankingResultKeys.FEATURED_DISCUSSION_IDS: [], RankingResultKeys.BRIEF_MENTION_ITEMS: [], RankingResultKeys.TOP_K_APPLIED: self.top_k, RankingResultKeys.EDITORIAL_NOTES: "No discussions to rank", RankingResultKeys.TOPIC_DIVERSITY: "N/A"}
            else:
                # Prepare for LLM
                discussions_summary = prepare_discussions_for_llm(discussions)

                # Rank with LLM (including anti-repetition and tracing)
                ranking_result = rank_with_llm(
                    discussions_summary,
                    self.summary_format,
                    previous_newsletter_context=self.previous_newsletter_context,
                ...
                ranking_result = apply_mmr_reranking(ranking_result, discussions, self.enable_mmr, self.mmr_lambda)

                # Apply top-k categorization
                ranking_result = apply_top_k_categorization(ranking_result, self.top_k, discussions)
            ...
            logger.error(f"Unexpected error ranking discussions: {e}, discussions_file={discussions_file}, output_file={output_file}")
            raise RuntimeError(f"Failed to rank discussions: {e}") from e
...
def rank_discussions(
    ...
    """
    Convenience function to rank discussions.

    This is a functional interface for simple use cases.
    ...
    ranker = DiscussionRanker(
        summary_format=summary_format,
        top_k=top_k,
        previous_newsletter_context=previous_newsletter_context,
        trace_id=trace_id,
        session_id=session_id,
        user_id=user_id,
    ...
    return ranker.rank(discussions_file, output_file, force_refresh)
