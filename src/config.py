...
class RAGSettings(BaseSettings):
    ...
    mcp_key_reauth_ttl_seconds: int = Field(
        default=45, ge=0, description="TTL (seconds) for re-authorizing a key on the Streamable HTTP MCP transport. A revoked/rotated key must not keep working, so authorize_current_tool re-resolves it against the live enabled flag once the resolved record is older than this. On the stateless transport records are resolved fresh per request, so this is a cheap backstop. 0 = re-resolve every call. Env: RAG_MCP_KEY_REAUTH_TTL_SECONDS."
    )
    mcp_allowed_hosts: str = Field(default="", description="Comma-separated Host allowlist for the Streamable HTTP MCP transport (MCP SDK DNS-rebinding guard). nginx/Cloudflare forward Host: mcp.langrag.ai; the SDK default allowlist is localhost-only and 421s that host. Set the public host(s) here to allow them. Empty string DISABLES DNS-rebinding protection, safe when nginx is the sole ingress and pins the vhost to this upstream. Env: RAG_MCP_ALLOWED_HOSTS.")
    ...
    mcp_public_url: str = Field(default=PODCAST_CONSUMER_MCP_URL_DEFAULT, description="Public MCP Streamable HTTP endpoint returned to a consumer on successful key verification (the URL they add to their MCP client). Env: RAG_MCP_PUBLIC_URL.")
    ...
    mcp_query_rate_per_min: int = Field(default=60, ge=1, description="In-process per-key sliding-window rate limit (requests/min) on the public podcast-MCP search path. Guards the MCP uvicorn (:8765), which slowapi does NOT cover. Over rate returns a clean tool error. Env: RAG_MCP_QUERY_RATE_PER_MIN.")
