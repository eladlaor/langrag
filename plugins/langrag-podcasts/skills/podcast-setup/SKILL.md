...
(Streamable HTTP). **No API key is required**: keyless access works out of the
box with a daily per-IP quota. An optional free API key (read from the
`LANGRAG_MCP_API_KEY` environment variable) raises the daily limit. The
podcast content lives on the server and is updated there when new episodes are
released — the plugin never needs a content update.

## First-time setup (keyless — the default)

Nothing to configure. After installing the plugin:

1. **Verify.** Call the `mcp__langrag__list_podcasts` tool. A successful
...
## Optional: get an API key (higher daily limits)
...
4. **Verify** with `mcp__langrag__list_podcasts` as above.

## Troubleshooting

| Symptom | Cause | Fix |
|---|---|---|
| 401 / "Invalid or missing MCP API key" | A key IS set but mistyped/revoked (an unset variable falls back to keyless access, so a 401 always means a bad presented key) | Fix or unset `LANGRAG_MCP_API_KEY` and restart; if the key is lost, redo the email verification at https://langrag.ai/podcasts — re-verifying ROTATES the key (the old one stops working) |
| Keyless quota error ("per IP") | The daily keyless per-IP budget is exhausted | Get a free key at https://langrag.ai/podcasts for the higher per-key budget, or wait for the UTC day to roll over |
